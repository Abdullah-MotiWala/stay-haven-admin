...
const navigate = useNavigate();
  ...
  return (
    ...
    <MatrixCard showshadow="true" icon={home}/>
     ...
     <div className="p-0 gap-[2px] inline-flex  max-w-full overflow-hidden rounded-lg">
        {roomTypes.map((type, index) => (
          <button
            key={type}
            onClick={() => setActiveType(type)}
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
    ...
    <img src={search} className="w-4 h-4" />
       ...
       <input type="text" className="border boeder-2 rounded-md bg-[#F9FAFC] font-medium px-5 py-2" placeholder="Search"/>
        ...
        <button
          onClick={() => setShowFilter(!showFilter)}
          className={`border px-4 py-2 rounded-md text-sm transition-all flex items-center gap-2 ${
            showFilter ? "bg-blue-50 border-blue-200 text-blue-600" : "bg-white text-gray-600"
          }`}
        >
