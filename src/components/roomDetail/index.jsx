...
function RoomDetail({ room }) {
  ...
  const displayImages = gallery.slice(0, 2);
    ...
    <div className="w-full max-w-2xl mx-auto bg-white p-4 md:p-8 rounded-[20px] md:rounded-[30px] shadow-sm font-sans overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-4">
        <div className="flex justify-center    items-center gap-2 md:gap-4">
          ...
          <span className="bg-[#90E6A7] text-[#1A4D2E] px-2 py-0.5 md:px-3 md:py-1 rounded-md text-[13px] md:text-xs font-bold uppercase whitespace-nowrap">
            {room.status}
          </span>
        </div>
      ...
      <p className="text-[#7C8DB5] text-[14px] font-medium flex items-center gap-1 mb-6 text-sm md:text-base">
        <span className="text-lg"><img src={location} alt="" className='size-3' /></span> {room.view}
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-8">
        <div className="lg:col-span-3">
          <img
            src={cardImage}
            alt="Main"
            className="w-full h-full md:h-[217px] md:width-[414px] object-cover rounded-[15px] md:rounded-[25px]"
          />
        </div>
        ...
        <div className="hidden lg:flex flex-col gap-3">
          {displayImages.map((img, index) => (
            <img
              key={index}
              src={cardImage}
              className="w-full h-[66px] object-cover rounded-[15px]"
              alt="thumb"
            />
          ))}
          {hasMore && (
            <div className="relative w-full h-[66px] rounded-[15px] overflow-hidden group cursor-pointer">
              <img src={cardImage} className="w-full h-full object-cover" alt="More" />
      ...
      {/* Description */}
      <p className="text-[#4B5563] leading-relaxed mb-2 text-sm md:text-[14px]">
        {room.description}
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-3">
        {/* Features */}
        <section className='m-2 p-0'>
          <h3 className="text-lg md:text-[24px] font-semibold mb-3 text-[#111827]">Features</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
            {["Designed with a spacious layout", "Private balcony", "Dedicated workspace", "Comfortable stay"].map((item) => (
              <div key={item} className="flex items-center gap-3 text-[14px] md:text-sm font-medium text-[#111827]">
                <img src={tick} alt='' className="height-5 width-5 bg-green-300 p-1  rounded-full" />
                {item}
              </div>
            ))}
          </div>
        ...
        {/* Amenities Icons */}
        <section className='m-2 p-0'>
          <h3 className="text-lg md:text-[24px] font-semibold mb-5 text-[#111827]">Amenities</h3>
          <div className="flex flex-wrap gap-1">
            <div className='p-1 bg-[#E7E7E7] rounded-xl items-center justify-center flex flex-col w-23 h-15'>
              <img src={wifi} alt="" className='size-4' />
              <p className='font-medium mb-0 text-[12px]'>Free Wifi</p>
            </div>
            <div className='p-1 bg-[#E7E7E7] rounded-xl items-center justify-center flex flex-col w-23 h-15'>
              <img src={group} alt="" className='size-4' />
              <p className='font-medium mb-0 text-[12px]'>Breakfast</p>
            </div> <div className='p-1 bg-[#E7E7E7] rounded-xl items-center justify-center flex flex-col w-23 h-15'>
              <img src={pool} alt="" className='size-4' />
              <p className='font-medium mb-0 text-[12px]'>Pool</p>
            </div>
            <div className='p-1 bg-[#E7E7E7] rounded-xl items-center justify-center flex flex-col w-23 h-15'>
              <img src={water} alt="" className='size-4' />
              <p className='font-medium mb-0 text-[12px]'>warm/cold water</p>
            </div>
      ...
      <div className="mt-12 pt-8 border-t border-gray-100">
        <h3 className="text-[24px] md:text-xl font-semibold mb-4 text-[#111827]">Room Facilities</h3>
