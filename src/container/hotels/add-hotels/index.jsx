...
const AddHotelForm = () => {
  ...
  return (
    <>
      <Navbar />
      <form className="min-h-screen w-full md:p-8 font-sans" onSubmit={handleSubmit}>
        <div className="max-full mx-auto mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Add New Hotel</h1>
          <p className="text-sm text-gray-500 font-medium">Enter hotel details and select options</p>
        </div>
        ...
        <div className="max-full mx-auto bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-6 md:p-[10%] pt-2 ">
            ...
            {/* Amenities Section */}
            <div className="mb-10">
              <h3 className="text-base font-bold text-gray-900 mb-6">Amenities Included</h3>
                ...
                {amenitiesList.map((item) => {
                  ...
                  return (
                    <label key={item} className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className={`text-sm font-bold ${checked ? "text-[#2563eb]" : "text-gray-400"}`}>{item}</span>
                    </label>
                  );
                })}
              </div>
            ...
            {/* Rooms Section */}
            <div className="mb-4">
              <h3 className="text-base font-bold text-gray-900 mb-6">Rooms Included</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4">
                {roomsList.map((room) => {
                  ...
                  return (
                    <label key={room} className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        className="w-5 h-5 rounded border-gray-300 text-[#2563eb]"
                      />
                      <span className={`text-sm font-bold ${checked ? "text-[#2563eb]" : "text-gray-400"}`}>{room}</span>
                    </label>
                  );
                })}
              </div>
        ...
        <div className="max-w-6xl mx-auto mt-10 flex justify-end gap-4">
          <button type="button" onClick={() => navigate(-1)} className="px-10 py-3 bg-white border rounded-full text-gray-600 font-bold hover:bg-gray-50">Back</button>
          <button type="submit" disabled={loading} className="px-10 py-3 bg-[#2563eb] text-white rounded-full font-bold hover:bg-blue-700 disabled:bg-gray-400 transition-all">
