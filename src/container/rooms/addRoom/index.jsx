...
const AddNewRoom = () => {
  ...
  useEffect(() => {
    ...
    const fetchHotel = async () => {
      setFetching(true);
      try {
        const res = await getById(id); // Fetching data using your API call
        const hotel = res.data;

        // Setting the fetched data into form fields
        form.setFieldsValue({
          name: hotel.roomName || "",
          ...
          maxAdults: hotel.maxAdults || 1,
          maxChildren: hotel.maxChildren || 0,
          pricePerNight: hotel.pricePerNight || 0.0,
          status: hotel.status || "available", // default status to 'available'
          description: hotel.description || "",
          mainImage: hotel.mainImage || "", // Set default value if main image is missing
          galleryImages: hotel.galleryImages || [], // Set default to empty array
          hotelId: hotel.hotelId || "",
          featureIds: hotel.featureIds || [],
        });
        setHotel(hotel); // Storing hotel data in state for further use if necessary
      } catch (err) {
